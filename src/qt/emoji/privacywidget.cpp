...
#include "qt/emoji/privacywidget.h"
#include "qt/emoji/forms/ui_privacywidget.h"
#include "qt/emoji/qtutils.h"
#include "guiutil.h"
#include "qt/emoji/denomgenerationdialog.h"
#include "qt/emoji/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(EMOJIGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zEMOJ anonymizes your EMOJ by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zEMOJ or convert back to EMOJ"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 EMOJ ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zEMOJ");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zEMOJ");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zEMOJ");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zEMOJ");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zEMOJ");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zEMOJ");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zEMOJ");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zEMOJ");
    ...
    // Only Convert to EMOJ enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzEMOJ->setTitleClassAndText("btn-title-grey", "Total 0 zEMOJ");
    ui->btnTotalzEMOJ->setSubTitleClassAndText("text-subtitle", "Show denominations of zEMOJ owned.");
    ui->btnTotalzEMOJ->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select EMOJ outputs to mint into zEMOJ.");
    ...
    connect(ui->btnTotalzEMOJ, SIGNAL(clicked()), this, SLOT(onTotalZemojClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zEMOJ");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of EMOJ to mint into zEMOJ"));
    }else{
        btnText = tr("Convert back to EMOJ");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zEMOJ to convert back into EMOJ"));
    }
...
void PrivacyWidget::onTotalZemojClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzEMOJ->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzEMOJ->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zEMOJ is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zEMOJ, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zEMOJ").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zEMOJ minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZemoj(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zEMOJ converted back to EMOJ"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEMOJ </b>";
    ...
    ui->btnTotalzEMOJ->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
