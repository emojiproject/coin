...
#include "qt/emoji/dashboardwidget.h"
#include "qt/emoji/forms/ui_dashboardwidget.h"
#include "qt/emoji/sendconfirmdialog.h"
#include "qt/emoji/txrow.h"
...
DashboardWidget::DashboardWidget(EMOJIGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of EMOJ staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareEmoj, "square-chart-emoj");
    setCssProperty(ui->labelSquarezEmoj, "square-chart-zemoj");
    setCssProperty(ui->labelEmoj, "text-chart-emoj");
    setCssProperty(ui->labelZemoj, "text-chart-zemoj");
    ui->labelZemoj->setVisible(false);
    ui->labelSquarezEmoj->setVisible(false);
    ...
    ui->labelAmountZemoj->setText("0 zEMOJ");
    ui->labelAmountZemoj->setVisible(false);
    ui->labelAmountEmoj->setText("0 EMOJ");
    setCssProperty(ui->labelAmountEmoj, "text-stake-emoj-disable");
    setCssProperty(ui->labelAmountZemoj, "text-stake-zemoj-disable");
    ...
    ui->btnHowTo->setText(tr("How to get EMOJ"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("EMOJ")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isEmoj = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZEMOJ;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair EMOJ, zEMOJ
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            emoj = (pair.first != 0) ? pair.first / 100000000 : 0;
            zemoj = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalEmoj += pair.first;
            chartData->totalZemoj += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesEmoj.append(emoj);
        chartData->valueszEmoj.append(zemoj);

        int max = std::max(emoj, zemoj);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("EMOJ");
    set1 = new QBarSet("zEMOJ");
    ...
    set0->append(chartData->valuesEmoj);
    set1->append(chartData->valueszEmoj);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalEmoj > 0 || chartData->totalZemoj > 0) {
        setCssProperty(ui->labelAmountEmoj, "text-stake-emoj");
        setCssProperty(ui->labelAmountZemoj, "text-stake-zemoj");
    } else {
        setCssProperty(ui->labelAmountEmoj, "text-stake-emoj-disable");
        setCssProperty(ui->labelAmountZemoj, "text-stake-zemoj-disable");
    }
    forceUpdateStyle({ui->labelAmountEmoj, ui->labelAmountZemoj});
    ui->labelAmountEmoj->setText(GUIUtil::formatBalance(chartData->totalEmoj, nDisplayUnit));
    ui->labelAmountZemoj->setText(GUIUtil::formatBalance(chartData->totalZemoj, nDisplayUnit, true));
