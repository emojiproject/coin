...
#include "ui_zemojcontroldialog.h"
...
std::set<std::string> ZEmojControlDialog::setSelectedMints;
std::set<CMintMeta> ZEmojControlDialog::setMints;

bool CZEmojControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZEmojControlDialog::COLUMN_DENOMINATION || column == ZEmojControlDialog::COLUMN_VERSION || column == ZEmojControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZEmojControlDialog::ZEmojControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZEmojControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zEMOJ Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZEmoj->setProperty("cssClass", "text-main-purple");
    ui->labelZEmoj_int->setProperty("cssClass", "text-main-purple");
...
ZEmojControlDialog::~ZEmojControlDialog()
{
    delete ui;
}

void ZEmojControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEmojControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZEmojControlWidgetItem* itemDenom(new CZEmojControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZEmojControlWidgetItem *itemMint = new CZEmojControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zemojTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zemojTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zEMOJ.";
            else if (!mint.isSeedCorrect)
                strReason = "The zEMOJ seed used to mint this zEMOJ is not the same as currently hold in the wallet";
            else
...
void ZEmojControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEmojControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZEmoj_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZEmojControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZEmojControlDialog::GetSelectedMints()
{
...
void ZEmojControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
