...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::EMOJ);
    ...
    if (settings.contains("nAnonymizeEMOjiAmount") || reset)
        SoftSetArg("-anonymizeemojiamount", settings.value("nAnonymizeEMOjiAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in emoji.cpp during shotdown to also remove the window positions
