...
*Setup instructions for a gitian build of Emoj Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the EMOJI
...
to the EMOJI GitHub release page.
...
- [Building Emoj Coin](#building-emoji-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/emoji-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/emoji-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/emoji-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/emoji-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/emoji-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/emoji-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/EMOji/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@emoji.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/emoji-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/emoji-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/emoji-osx-*-build.assert
```
