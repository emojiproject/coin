// Copyright (c) 2018-2019 The EMOJI developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zemoj/deterministicmint.h>
#include "zemojtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzEMOJTracker::CzEMOJTracker(std::string strWalletFile)
{
...
CzEMOJTracker::~CzEMOJTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzEMOJTracker::Init()
{
...
bool CzEMOJTracker::Archive(CMintMeta& meta)
{
...
bool CzEMOJTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzEMOJTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzEMOJTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzEMOJTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzEMOJTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzEMOJTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzEMOJTracker::GetSerialHashes()
{
...
CAmount CzEMOJTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzEMOJTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzEMOJTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzEMOJTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzEMOJTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzEMOJTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzEMOJTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzEMOJTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzEMOJTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzEMOJTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzEMOJTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzEMOJWallet* zEMOJWallet)
{
    bool iszEMOJWalletInitialized = (NULL != zEMOJWallet);
    CMintMeta meta;
    ...
    if (! iszEMOJWalletInitialized)
        zEMOJWallet = new CzEMOJWallet(strWalletFile);
    meta.isSeedCorrect = zEMOJWallet->CheckSeed(dMint);
    if (! iszEMOJWalletInitialized)
        delete zEMOJWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzEMOJTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzEMOJTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzEMOJTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzEMOJTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzEMOJTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzEMOJTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzEMOJWallet* zEMOJWallet = new CzEMOJWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zEMOJWallet);
        }
        delete zEMOJWallet;
        LogPrint("zero", "%s: added %d dzemoj from DB\n", __func__, listDeterministicDB.size());
    }
