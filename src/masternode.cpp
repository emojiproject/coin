...
{
    if (nHeight >  2000000) return     0 * COIN;
    if (nHeight >  1000000) return     10 * COIN;
    if (nHeight >   500000) return     25 * COIN;
    if (nHeight >   250000) return     50 * COIN;
    if (nHeight >        2) return     100 * COIN;
    if (nHeight >        1) return     95000000 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 EMOJ tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 EMOJ tx -> 1 confirmation
