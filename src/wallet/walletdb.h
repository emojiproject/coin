...
public:
    ...
    bool WriteZEMOJSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZEMOJSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZEMOJSeed_deprecated(uint256& seed);
    bool EraseZEMOJSeed();
    bool EraseZEMOJSeed_deprecated();

    bool WriteZEMOJCount(const uint32_t& nCount);
    bool ReadZEMOJCount(uint32_t& nCount);
