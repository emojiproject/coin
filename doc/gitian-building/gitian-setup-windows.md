...
The build script we'll be using is contained in the EMOJI github repository ([contrib/gitian-build.py](https://github.com/EMOji/emoji/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the EMOJI repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/EMOji/EMOJI/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, emoji-detached-sigs, and emoji GitHub repos.
