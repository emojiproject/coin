...
#include "qt/emoji/forms/ui_addresseswidget.h"
#include "qt/emoji/addresslabelrow.h"
#include "qt/emoji/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(EMOJIGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a EMOJI address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress emojAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(emojAdd.Get(), label.toUtf8().constData(),
                emojAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
