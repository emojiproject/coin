...
CzEMOJWallet::CzEMOJWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zemoj seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZEMOJSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZEMOJSeed_deprecated()) {
                    LogPrintf("%s: Updated zEMOJ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zemoj seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zemoj wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzEMOJWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZEMOJCount(nCountLastUsed);
    else if (!walletdb.ReadZEMOJCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzEMOJWallet::Lock()
{
    seedMaster = 0;
}

void CzEMOJWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzEMOJWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZEMOJ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzEMOJWallet::LoadMintPoolFromDB()
{
...
void CzEMOJWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzEMOJWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzEMOJWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zemojTracker->HasPubcoinHash(pMint.first)) {
...
bool CzEMOJWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZEMOJ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zemojTracker which also adds to database
    pwalletMain->zemojTracker->Add(dMint, true);
...
void CzEMOJWallet::SeedToZEMOJ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzEMOJWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzEMOJWallet::GenerateDeterministicZEMOJ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzEMOJWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzEMOJWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzEMOJWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
