...
#include "qt/emoji/send.h"
#include "qt/emoji/forms/ui_send.h"
#include "qt/emoji/addnewcontactdialog.h"
#include "qt/emoji/qtutils.h"
#include "qt/emoji/sendchangeaddressdialog.h"
#include "qt/emoji/optionbutton.h"
#include "qt/emoji/sendconfirmdialog.h"
#include "qt/emoji/myaddressrow.h"
#include "qt/emoji/guitransactionsutils.h"
...
#include "zemojcontroldialog.h"

SendWidget::SendWidget(EMOJIGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("EMOJ");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zEMOJ");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (EMOJ)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a EMOJI address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 EMOJ");
    ...
    setCssProperty(coinIcon, "coin-icon-emoj");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onEMOJSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onEMOJSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send EMOJ");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zEMOJ");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZemoj = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZemoj));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZemoj ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendEmoj = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendEmoj ? AskPassphraseDialog::Context::Send_EMOJ : AskPassphraseDialog::Context::Send_zEMOJ, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendEmoj) ? send(recipients) : sendZemoj(recipients)) {
...
bool SendWidget::sendZemoj(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zEMOJ is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zEMOJ selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZEmojControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZEmojControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZemoj(
        ...
        inform(tr("zEMOJ transaction sent!"));
        ZEmojControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZEMOJ_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zEMOJ require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zEMOJ transaction
        ...
        emit message("zEMOJ transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isEMOJ){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any EMOJ to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZEmojControlDialog *zEmojControl = new ZEmojControlDialog(this);
            zEmojControl->setModel(walletModel);
            zEmojControl->exec();
            ui->btnCoinControl->setActive(!ZEmojControlDialog::setSelectedMints.empty());
            zEmojControl->deleteLater();
        } else {
            inform(tr("You don't have any zEMOJ in your balance to select."));
        }
    }
}
...
void SendWidget::onEMOJSelected(bool _isEMOJ){
    isEMOJ = _isEMOJ;
    setCssProperty(coinIcon, _isEMOJ ? "coin-icon-emoj" : "coin-icon-zemoj");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress emojAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(emojAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(emojAdd.Get(), dialog->getLabel().toStdString(), "send")) {
