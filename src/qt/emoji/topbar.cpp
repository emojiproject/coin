...
#include "qt/emoji/topbar.h"
#include "qt/emoji/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/emoji/lockunlock.h"
#include "qt/emoji/qtutils.h"
#include "qt/emoji/receivedialog.h"
...
TopBar::TopBar(EMOJIGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopEmoj, ui->labelAmountTopzEmoj}, "amount-small-topbar");
    setCssProperty({ui->labelAmountEmoj, ui->labelAmountzEmoj}, "amount-topbar");
    setCssProperty({ui->labelPendingEmoj, ui->labelPendingzEmoj, ui->labelImmatureEmoj, ui->labelImmaturezEmoj}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzEmoj->setVisible(false);
    ui->labelAmountzEmoj->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzEmoj->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezEmoj->setVisible(false);
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("EMOJI")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount emojAvailableBalance = balance + delegatedBalance;

    // zEMOJ Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalEmoj = GUIUtil::formatBalance(emojAvailableBalance, nDisplayUnit);
    QString totalzEmoj = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopEmoj->setText(totalEmoj);
    ui->labelAmountTopzEmoj->setText(totalzEmoj);

    // Expanded
    ui->labelAmountEmoj->setText(totalEmoj);
    ui->labelAmountzEmoj->setText(totalzEmoj);

    ui->labelPendingEmoj->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzEmoj->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureEmoj->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezEmoj->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
