...
CZEmojStake::CZEmojStake(const libzerocoin::CoinSpend& spend)
{
...
int CZEmojStake::GetChecksumHeightFromMint()
{
...
int CZEmojStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZEmojStake::GetChecksum()
{
    return nChecksum;
}

// The zEMOJ block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZEmojStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZEmojStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zEMOJ staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZEmojStake::GetUniqueness()
{
    //The unique identifier for a zEMOJ is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZEmojStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZEmojStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zEMOJ that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZEMOJOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zEMOJ output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zEMOJ", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZEMOJOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zEMOJ output", __func__);
...
bool CZEmojStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZEmojStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzEMOJTracker* zemojTracker = pwallet->zemojTracker.get();
    CMintMeta meta;
    if (!zemojTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zemojTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!EMOJ Stake
bool CEmojStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CEmojStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CEmojStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CEmojStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CEmojStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CEmojStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CEmojStake::GetUniqueness()
{
    //The unique identifier for a EMOJ stake is the outpoint
...
CBlockIndex* CEmojStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
